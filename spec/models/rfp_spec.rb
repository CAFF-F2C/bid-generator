...
RSpec.describe Rfp, type: :model do
  subject(:rfp) { create(:rfp, buyer: buyer, start_year: 2021, procurement_type: procurement_type) }
  ...
  it { is_expected.to belong_to(:procurement_type).inverse_of(:rfps) }
  ...
  it { is_expected.to validate_presence_of(:procurement_type) }

  describe '#name' do
    it 'builds the name from year and type' do
      expect(rfp.name).to eq('TestType (2021 - 2022)')
    end
  end
